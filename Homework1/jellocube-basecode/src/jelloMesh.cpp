...
#define COLLISION_EPSILON .05

// TODO
double JelloMesh::g_structuralKs = 3000.0; //stretch constant
double JelloMesh::g_structuralKd = 5.0;  //dampening constant
...
double JelloMesh::g_penaltyKs = 75000.0;
	...
	for (int i = 0; i < m_rows + 1; i++)
	{
		for (int j = 0; j < m_cols + 1; j++)
		{
			for (int k = 0; k < m_stacks + 1; k++)
			{
				...
				if (i < m_rows - 1 && j < m_cols - 1) AddShearSpring(GetParticle(g, i, j, k), GetParticle(g, i + 2, j + 2, k));
				if (i< m_rows - 1 && j>1) AddShearSpring(GetParticle(g, i, j, k), GetParticle(g, i + 2, j - 2, k));
				if (i>1 && k < m_stacks - 1) AddShearSpring(GetParticle(g, i, j, k), GetParticle(g, i - 2, j, k + 2));
				if (j < m_cols - 1 && k < m_stacks - 1) AddShearSpring(GetParticle(g, i, j, k), GetParticle(g, i, j + 2, k + 2));
				if (i < m_rows - 1 && k < m_stacks - 1) AddShearSpring(GetParticle(g, i, j, k), GetParticle(g, i + 2, j, k + 2));
				if (j > 1 && k < m_stacks - 1) AddShearSpring(GetParticle(g, i, j, k), GetParticle(g, i, j - 2, k + 2));
...
void JelloMesh::ComputeForces(ParticleGrid& grid)
{
	...
	for (unsigned int i = 0; i < m_vsprings.size(); i++)
	{
		...
		//vec3 diffv = b.velocity - a.velocity;
		vec3 diffv = a.velocity - b.velocity;
		double dist = diffp.Length();
		double Ks = spring.m_Ks;
		double Kd = spring.m_Kd;

		vec3 force = (-(Ks *(dist - spring.m_restLen) + (Kd*Dot(diffp, diffv) / dist))*diffp / dist);

		a.force += force;
		b.force += -force;
		//set bends as the highest.kds less than 10.kss in the thousands
...
void JelloMesh::ResolveContacts(ParticleGrid& grid)
{
	for (unsigned int i = 0; i < m_vcontacts.size(); i++)
	{
		for (unsigned int i = 0; i < m_vcontacts.size(); i++)
		{
			...
			p.force += contact.m_distance * g_penaltyKs * normal;
			//applying the force to contacts. stiff spring frce for penetration	
		}
	}
}
...
void JelloMesh::ResolveCollisions(ParticleGrid& grid)
{
	for (unsigned int i = 0; i < m_vcollisions.size(); i++)
	{
		...
		double r = 1; //a higher restitution denatures.

		pt.velocity = pt.velocity - 2 * (Dot(pt.velocity, normal)*r*normal);
		// calculates the mirror velocity. i.e pool table. collision forcefint
...
{
	// TODO
	if (p.position[1] < 0.0) //If the particle is below y=0, call it a 'CONTACT'
	{
		intersection.m_p = p.index;   //header file def.
		intersection.m_distance = -p.position[1];
		intersection.m_type = CONTACT;
		intersection.m_normal = vec3(0.0, 1.0, 0.0);
		return true;
	}
	else if (p.position[1] < COLLISION_EPSILON)
	{
	...
	JelloMesh::Intersection& result)
{
	...
	else if (dist < cylinderRadius + COLLISION_EPSILON)
	{
		result.m_p = p.index;
		result.m_distance = cylinderRadius;
...
bool JelloMesh::SphereIntersection(Particle& p, World::Sphere* sphere,
	JelloMesh::Intersection& result) //add in header
{
	...
	if (normal.Length() < radius + COLLISION_EPSILON)
	{
		result.m_p = p.index;
		result.m_distance = radius;
		result.m_type = COLLISION;
	...
	for (int i = 0; i < m_rows + 1; i++)
	{
		for (int j = 0; j < m_cols + 1; j++)
		{
			for (int k = 0; k < m_stacks + 1; k++)
			{
				...
				//2nd law // K1= F(t,y)= dt
				p.position = p.velocity + dt * p.velocity;
				// deriv of velocity. numerical integration of velocity // y= y+h * K1
	...
	for (int i = 0; i < m_rows + 1; i++)
	{
		for (int j = 0; j < m_cols + 1; j++)
		{
			for (int k = 0; k < m_stacks + 1; k++)
			{
				...
				//           y              h/2          k1         k2
				//p.velocity = p.velocity + (dt*0.5) * (p.force + t.force);
				//p.position = p.position + (dt*0.5) * (p.velocity + t.velocity);
